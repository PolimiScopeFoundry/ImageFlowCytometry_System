...
class IfcMeasureTriggered(Measurement):
 
    name = "IFC_measurement_triggered"
    ...
    def save_roi(self):
        ...
        for camera in self.cameras:
            ...
            camera.settings['trigger_source'] = 'External'
            camera.camera_device.start_acquisition(buffersize=self.settings.buffer_size.val)

        time_index=0
        self.settings['detect'] = True 

        # TODO: start trigger - ROI uses continous mode
        self.trigger.settings['run_mode'] = 'continuous'  
        self.trigger.settings['freq'] = self.settings['ni_freq'] 
        self.trigger.settings['duty_cycle'] = self.settings['ni_duty_cycle']
        self.trigger.settings['initial_delay'] = self.settings['ni_initial_delay']
        ...
        while not self.interrupt_measurement_called:
            ...
            # robust check: frame pairing consistency (log only; do not crash)
            if (frame_idA is not None) and (frame_idB is not None) and (frame_idA != frame_idB):
                self.log.warning(f"Frame ID mismatch: camA={frame_idA}, camB={frame_idB}")
            ...
            for roi_idx in range(len(im.cx)):
                ...
                if self.interrupt_measurement_called or time_index >= self.settings.rois_per_file.val:
                    ...
                    for camera in self.cameras:
                        camera.settings['trigger_source'] = 'Internal'
                    self.close_h5()
    ...
    def save_stack(self):
        ...
        # TODO: start trigger - Stack uses finite mode
        self.trigger.settings['run_mode'] = 'finite'
        self.trigger.settings['freq'] = self.settings['ni_freq']
        self.trigger.settings['duty_cycle'] = self.settings['ni_duty_cycle']
        self.trigger.settings['initial_delay'] = self.settings['ni_initial_delay']
        self.trigger.settings['n_pulses'] = int(znum)
        ...
        while self.frame_index < znum:
            ...
            imgA = self.cameras[0].camera_device.get_frame()
            _, _, _, _, _, frame_idA = self.cameras[0].camera_device.get_buffer_count()

            imgB = self.cameras[1].camera_device.get_frame()
            _, _, _, _, _, frame_idB = self.cameras[1].camera_device.get_buffer_count()
