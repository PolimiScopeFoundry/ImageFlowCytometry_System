...
frame_index = 90   # ToDo: check frame 0 and 7
...
t0 = time.time()
im.find_object_localmax_NMS(
    channel=0,
    bitdepth=12,
    norm_factor=16,
    intensity_threshold=10,  # tune for SNR
    nms_d_min=50,             # min distance between peaks (px)
    R_cluster=100,            # cluster radius (px)
    N_cluster=20            # neighbors >= 200 => aggregate
)
